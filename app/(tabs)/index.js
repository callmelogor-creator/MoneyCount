...
export default function Index() {
  ...
  return (
    <SafeAreaProvider>
      {/* 最外層 View 強制黑色背景，並移除所有內距 */}
      <View style={styles.container}>
        <StatusBar barStyle="light-content" backgroundColor="rgba(0,0,0,0)" translucent={true} />
        
        {/* 背景圖層級：使用負 margin 強制覆蓋狀態列位置 */}
        <ImageBackground 
          source={MY_CUSTOM_BACKGROUND} 
          style={styles.bgImage}
          imageStyle={{ top: 0 }}
        >
          <View style={styles.overlay}>
            <View style={{ flex: 1 }}>
              {/* Header 調整：手動控制頂部距離，避開時間電量圖示 */}
...
const styles = StyleSheet.create({
  ...
  // 2. ImageBackground 設定負 margin，強制向上推到最頂端，寬高強制填滿
  bgImage: { 
    flex: 1, 
    width: '100%',
    height: '100%',
    marginTop: 0, // 如果還有白邊，可以試著設為 -StatusBar.currentHeight
  },
  overlay: { 
    flex: 1, 
  ...
  // 3. 手動控制頂部 Padding，這會覆蓋狀態列位置，不留白
  stickyHeader: { 
  ...
  cyberCard: { backgroundColor: 'rgba(0,0,0,0.7)', borderRadius: 20, padding: 15, borderWidth: 1, borderColor: '#333', marginTop: 10 },
